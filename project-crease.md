...
To simplify this expression, we evaluate the last term using the formula for the sum of a geometric series ($\sum_{i=0}^{k} ar^{i} = S = \frac{a \cdot (1 - r^{k+1})}{1 - r}$, where here our $a=n^{2}$, $r=\frac{1}{2}$, and $k=\log _{2}{n} -1$)
