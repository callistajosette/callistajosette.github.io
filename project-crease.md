...
Unrolling a recurrence is a strategy we typically use when the recursive case only has one recursive call. That is, if the recursive case looks like T(n)=T(n/2)+[nonrecursive work],for example.
