...
What is $\ell$, the height of our tree? Note that reaching $\ell$ means we have performed sufficient splits to reach our base case (where each subproblem consists of only one digit). Then it must be that...
<center>
$$ 
\begin{align*}
~\frac{n}{2^{\ell}} & \leq 1 \\
